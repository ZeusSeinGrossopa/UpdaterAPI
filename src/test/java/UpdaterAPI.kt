...
object UpdaterAPI {
    ...
    private const val GITHUB_CUSTOM_URL = "https://api.github.com/repos/%s/releases/latest"
    ...
    fun downloadUpdater(destination: File, consumer: Consumer<File?>? = null) {
        ...
        destination = File((if (destination.isDirectory) destination else File(FilenameUtils.getPath(destination.absolutePath))).toString() + "/Updater.jar")
    ...
    private fun getLatestReleaseFromGithub(githubUser: String, repository: String, consumer: Consumer<Array<String?>>) {
        try {
