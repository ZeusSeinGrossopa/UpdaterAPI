...
object UpdaterAPI {
    ...
    private val API: String? = "https://api.github.com/repos/ZeusSeinGrossopa/UpdaterAPI/releases/latest"
    private val GITHUB_CUSTOM_URL: String? = "https://api.github.com/repos/%s/releases/latest"
    ...
    fun downloadUpdater(destination: File?, consumer: Consumer<File?>? = null) {
        ...
        destination = File((if (destination.isDirectory()) destination else File(FilenameUtils.getPath(destination.getAbsolutePath()))).toString() + "/Updater.jar")
    ...
    fun getLatestReleaseFromGithub(githubUser: String?, repository: String?, consumer: Consumer<Array<String?>?>?) {
        try {
            ...
            if (connect.responseCode == 200) {
                ...
                val downloadLink = `object`.entrySet().stream().filter { e: MutableMap.MutableEntry<String?, JsonElement?>? -> e.key == "assets" }
    ...
    fun update(updaterFile: File?, oldFile: File?, url: String?, newFile: File?, restart: Boolean) {
        ...
        val builder = ProcessBuilder(javaBin, "-jar", updaterFile.getAbsolutePath(), url, oldFile.getAbsolutePath(), newFile.getAbsolutePath(), if (restart) "true" else "")
        if (autoDelete) {
            autoDelete = false
            downloadUpdater(oldFile.getParentFile()) { file: File? ->
                try {
                    builder.start()
                } catch (e: IOException) {
                    e.printStackTrace()
    ...
    private fun compareVersions(version1: String?, version2: String?): Int {
        val levels1: Array<String?> = version1.split("\\.".toRegex()).toTypedArray()
        val levels2: Array<String?> = version2.split("\\.".toRegex()).toTypedArray()
